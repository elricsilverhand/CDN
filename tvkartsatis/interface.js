( function ( $ ) {
        ...
        $.fn.cookiesLawAnnouncements = function () {
            if ( !$( this ).length || Cookies.get( 'tvkartsatis_cookies' ) === 'accepted' ) {
                var popup        = $( this ),
                    ...
                    var popup        = $( this ),
                        ...
                        showPopup    = function () {
                            ...
                            popup.on( 'click', '.pp_cookies_accept_btn', function ( e ) {
                                e.preventDefault();
                                popup_parent.removeClass( 'pp_onshow' ).addClass( 'pp_onhide' );
                                Cookies.set( 'tvkartsatis_cookies', 'accepted', { expires : stt.day_next, path : '/' } );
                            } );
                        };
                ...
                $.magnificPopup.open( {
                    ...
                    callbacks    : {
                        ...
                        open       : function () {
                            ...
                            el.flickity( JSON.parse( '{ "fade":true,"cellSelector": ".q-item:not(.is_varhide)","cellAlign": "center","wrapAround":true,"autoPlay": true,"prevNextButtons":true,"adaptiveHeight": true,"imagesLoaded": false, "lazyLoad": 0,"dragThreshold" : 0,"pageDots": true,"rightToLeft": false }' ) );
...
$(document).ready(function () {
    $("#mobile_live_search").keyup(function () {
        var query = $(this).val();
        if (query != "") {
            $.ajax({
                url: '/index/newarama/',
                ...
                success: function (data) {
                    $('#mobile_search_result').html(data);
                }
            });
        } else {
        }
...
function loadmore()
{
 ...
 $('.loadmorearea').css('display','none');
};

function loadmorekategori(kategorid)
{
 var val = document.getElementById("result_no").value;
 var kategorid = kategorid;
 $.ajax({
 type: 'post',
 url: '../index/loadmorekategori/',
 data: {
  rec:val,
  dataid:kategorid
 },
...
function loadmoremarka(markaid)
{
 var val = document.getElementById("result_no").value;
 var markaid = markaid;
 $.ajax({
 type: 'post',
 url: '../index/loadmoremarka/',
 data: {
  rec:val,
  dataid:markaid
 },
